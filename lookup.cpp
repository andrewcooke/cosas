...
Sine::Sine(float gamma) {
  for (size_t i = 0; i < quarter_table.size(); i++) {
    float shape = sin(2 * numbers::pi * i / full_wavetable_size);
...
Noise::Noise(int smooth) {
  ...
  if (smooth > 1) {
    array<int32_t, full_wavetable_size> smoothed;
    fill(smoothed.begin(), smoothed.end(), 0);
    for (size_t i = 0; i < full_wavetable_size; i++) {
      for (size_t j = 0; j < smooth; j++) {
	smoothed.at(i) += ((int32_t)full_table.at((i + j) % full_wavetable_size) - sample_zero);
      }
    }
