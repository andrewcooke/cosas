...
template<uint O, uint S> void __not_in_flash_func(EEPROM::write_cv_midi_note)(CC<O, S> cc, Channel lr, uint8_t note_num) {
	cc.write_cv(lr, midi_to_dac(note_num, lr) >> 8);  // 11 bits
}

template<uint O, uint S> void __not_in_flash_func(EEPROM::write_cv_midi_note)(CC<O, S> cc, uint lr, uint8_t note_num) {
