...
KnobChange Knob::handle_knob_change(uint16_t now, uint16_t prev) {
  normalized = std::max(0.0f, std::min(1.0f, normalized + 0.1f * static_cast<float>(now - prev) / 4095));
  // normalized = static_cast<float>(now) / 4095;
  // normalized += static_cast<float>(now - prev) / 4095;
